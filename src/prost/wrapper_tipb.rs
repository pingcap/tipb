...
impl Executor {
    ...
    pub fn clear_projection(&mut self) {
        self.projection = ::std::option::Option::None
    }
    pub fn set_projection(&mut self, v: Projection) {
        self.projection = ::std::option::Option::Some(v);
    }
    pub fn get_projection(&self) -> &Projection {
        match self.projection.as_ref() {
            Some(v) => v,
            None => <Projection as ::protobuf::Message>::default_instance(),
        }
    }
    pub fn mut_projection(&mut self) -> &mut Projection {
        if self.projection.is_none() {
            self.projection = ::std::option::Option::Some(Projection::default());
        }
        self.projection.as_mut().unwrap()
    }
    pub fn take_projection(&mut self) -> Projection {
        self.projection.take().unwrap_or_else(Projection::default)
    }
}
